 ...
 <title>transactions Form</title>
  ...
  <h1>transactions Form</h1>
   ...
   <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="cattid">CattleID:</label>
        <input type="number" id="cattid" name="cattid" required><br><br>
        ...
        <input type="number" id="transprice" name="transprice" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO Transaction (TransactionID, CattleID, TransactionType, Price) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $transID, $cattID, $transType, $transPrice);

        // Set parameters from POST data with validation (optional)
        $transID = intval($_POST['transid']); // Ensure integer for ID
        $cattID = intval($_POST['cattid']); 
    ...
    <table border="5">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $transid = $row['TransactionID']; // Fetch the aCustomerid
                echo "<tr>
                    <td>" . $row['TransactionID'] . "</td>
                    <td>" . $row['CattleID'] . "</td>
                    <td>" . $row['TransactionType'] . "</td>
                    <td>" . $row['Price'] . "</td>
                    <td><a style='padding:4px' href='delete_Transaction.php?TransactionID=$transid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_Transaction.php?TransactionID=$transid'>Update</a></td> 
                </tr>";
            }
        } else {
